...
| [`c/`](../languages/c/) | 156 lines, 6 node types | a C subset, the first two constructs of [ROADMAP 6](ROADMAP.md#6-a-c-compiler): `int main(){return 42;}` and then `+ - * /` with parentheses, compiled to arm64 assembly that `cc` assembles and links by a stack machine with no register allocator. **The one language on the roadmap as a goal rather than a mechanism.** 10 programs exit with what `cc` makes them exit with, and nothing in the directory has a hand-written expected result |
