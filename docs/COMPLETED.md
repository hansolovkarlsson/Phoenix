...
| [`c/`](../languages/c/) | 125 lines, 5 node types | a C subset, the first construct of [ROADMAP 6](ROADMAP.md#6-a-c-compiler): `int main(){return 42;}` compiled to arm64 assembly that `cc` assembles and links. **The one language on the roadmap as a goal rather than a mechanism.** 4 programs exit with what `cc` makes them exit with, and nothing in the directory has a hand-written expected result |
