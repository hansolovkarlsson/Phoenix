...
| [`c/`](../languages/c/) | 184 lines, 8 node types | a C subset, the first three constructs of [ROADMAP 6](ROADMAP.md#6-a-c-compiler): `int main(){return 42;}`, then `+ - * /` with parentheses, then unary minus and the six comparisons, compiled to arm64 assembly that `cc` assembles and links by a stack machine with no register allocator. **The one language on the roadmap as a goal rather than a mechanism.** 16 programs exit with what `cc` makes them exit with, and nothing in the directory has a hand-written expected result |
