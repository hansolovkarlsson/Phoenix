...
| [`c/`](c/) | a C subset, begun: `int main(){return 42;}`, arithmetic and comparison over constants and locals, `if`, `while` and `for`, functions with parameters and calls, `&` and `*` over pointer declarators, compiled to arm64 assembly that `cc` assembles and links. The one language on the roadmap as a goal rather than a mechanism, and `cc` is its oracle |
