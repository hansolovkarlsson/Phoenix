...
```sh
make            # bin/phx
make test       # 217 checks, covering 35 Pascal programs against fpc
                #   and every Solveig program in a checkout, byte for byte
```

C11 and no dependencies. **The suite passes with nothing outside this
repository** — 212 of the 217 need only what is vendored here, and it is worth
...
212 passed, 0 failed and 5 skipped. The C oracle needs `cc` on arm64, which is
