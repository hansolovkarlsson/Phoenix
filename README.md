...
```sh
make            # bin/phx, and bin/limit, which the tests run programs under
make test       # 321 checks, covering 35 Pascal programs against fpc
                #   and every Solveig program in a checkout, byte for byte
```

C11 and no dependencies. **The suite passes with nothing outside this
repository**: 316 of the 321 need only what is vendored here, and it is worth
...
316 passed, 0 failed and 5 skipped. The C oracle needs `cc` on arm64, which is
what this repository is built on, and is skipped elsewhere. The SolVM assembler
is in the 316: its programs are held against the bytes they assembled to last
...
ok    66 claims from docs/reference.md section 11 hold
ok    and the refusals it names
