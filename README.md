...
make test       # 322 checks, covering 35 Pascal programs against fpc
                #   and every Solveig program in a checkout, byte for byte
```

C11 and no dependencies. **The suite passes with nothing outside this
repository**: 317 of the 322 need only what is vendored here, and it is worth
...
317 passed, 0 failed and 5 skipped. The C oracle needs `cc` on arm64, which is
what this repository is built on, and is skipped elsewhere. The SolVM assembler
is in the 317: its programs are held against the bytes they assembled to last
